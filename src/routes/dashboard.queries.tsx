...
function QueriesPage() {
  ...
  const updateEstado = async (id: string, estado: "leido" | "respondido") => {
    const { error } = await supabase.from("consultas").update({ estado } as never).eq("id", id);
  ...
  return (
    <div className="space-y-4">
      <div>
        ...
        <p className="text-sm text-muted-foreground">Mensajes de clientes interesados en tus productos.</p>
      </div>
                ...
                <div className="min-w-0 flex-1">
                  <div className="flex items-center justify-between gap-2">
                    <div className="truncate text-sm font-medium">
                      ...
                      {c.productos?.nombre && (
                        <span className="font-normal text-muted-foreground"> · {c.productos.nombre}</span>
                      )}
                    </div>
                    <Badge variant={c.estado === "nuevo" ? "default" : c.estado === "respondido" ? "secondary" : "outline"}>
                      {c.estado}
                    </Badge>
                  </div>
                  <p className="line-clamp-1 text-sm text-muted-foreground">{c.mensaje}</p>
                  <p className="mt-1 text-xs text-muted-foreground">{format(new Date(c.created_at), "dd MMM yyyy HH:mm")}</p>
                </div>
              </button>
            ))}
          </CardContent>
            ...
            <>
              ...
              <div className="mt-4 space-y-3">
                <div className="text-xs text-muted-foreground">
                  {format(new Date(selected.created_at), "dd MMM yyyy HH:mm")} · canal {selected.canal}
                </div>
                ...
                <div className="flex gap-2 pt-2">
                  <Button variant="outline" className="flex-1" onClick={() => updateEstado(selected.id, "leido")}>
                    Marcar leída
                  </Button>
                  <Button className="flex-1" onClick={() => updateEstado(selected.id, "respondido")}>
                    <Check className="mr-2 h-4 w-4" />Respondida
                  </Button>
                </div>
              </div>
            </>
