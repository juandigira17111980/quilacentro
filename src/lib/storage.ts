 ...
 */
export async function uploadAndGetUrl(bucket: string, path: string, file: File): Promise<string> {
  const { error } = await supabase.storage
    .from(bucket)
    .upload(path, file, { upsert: true, contentType: file.type, cacheControl: "31536000" });
  if (error) throw error;
  const tenYears = 60 * 60 * 24 * 365 * 10;
  const { data, error: e2 } = await supabase.storage.from(bucket).createSignedUrl(path, tenYears);
